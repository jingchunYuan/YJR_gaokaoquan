...
-(void)drawTableBackGroundWithRect:(CGRect )rect{
    ...
    [unitYStr drawInRect:CGRectMake(distanceToLeft,0,130,22) withAttributes:attribute];
...
//        [self addSubview:showPoint];
    ...
    for (int i = 0 ; i < self.yValueArr.count ; i++) {
        ...
        [[NSString stringWithFormat:@"%ld",[self.yValueArr[i] integerValue]*100] drawInRect:CGRectMake(distanceToLeft,(self.yValueArr.count - i) * self.horizontalSpace*2,30,0.2*SCREEN_HEIGHT) withAttributes:attribute1];
    }
    
    //1.1绘制虚线
    for (int i = 0 ; i < self.yValueArr.count ; i++) {
        CGContextMoveToPoint(context, distanceToLeft + 35, 2*(self.yValueArr.count - i) * self.horizontalSpace + 8);
        CGContextAddLineToPoint(context, (self.xValueArr.count + 2.1) * (rect.size.width * verticalSpace), 2*(self.yValueArr.count - i) * self.horizontalSpace + 8);
        //这里为其单独设置虚线
        CGFloat lengths[] = {3,1};
    ...
    //2绘制横轴
    for (int i = 0 ; i < 6 ; i++) {
        [self.xValueArr[i] drawInRect:CGRectMake(i  * (rect.size.width * 0.12)+(rect.size.width * 0.14),(self.yValueArr.count + 1) * self.horizontalSpace*2 ,30 ,22) withAttributes:attribute];
    }
    //    NSString *unitXStr = @"(月)";
...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i = 0 ; i < self.animationArr.count ; i++) {
        ...
        for (int j = 0 ; j < subArr.count ; j++) {
            //            float y = (self.yValueArr.count -  [subArr[j] intValue] / (self.spaceValue * self.level/100.0) ) * self.horizontalSpace + 7;
            
            float y = (self.yValueArr.count*2 -  [subArr[j] intValue] / (self.spaceValue * self.level/100.0) ) * self.horizontalSpace*2 + 7;
            float x =  j * (rect.size.width * 0.12)+ 5 + rect.size.width * 0.14;
            //            float x =  (j + 2) * (rect.size.width * verticalSpace)+ 5 ;
            BLPointBtn *clickBtn = [[BLPointBtn alloc]initWithFrame:CGRectMake(x - 6, y/2 - 6, 12, 12)];
            [clickBtn setBackgroundImage:[UIImage imageNamed:@"gaoxiaochaxun-sanjiaoxing@3x"] forState:UIControlStateNormal];
...
-(void)getYDataArrWithValueArr:(NSArray *)valueArr{
    ...
    if((maxValue  / [self getUnitWithMaxValue:maxValue]) / (self.rowNum - 1) == 0){
        ...
        self.spaceValue = (maxValue  / [self getUnitWithMaxValue:maxValue]) / (self.rowNum - 1)*100;
    }
    ...
    [self.yValueArr addObjectsFromArray:@[@"0",@"2",@"4",@"6",@"8"]];
...
-(void)pointClick:(BLPointBtn *)clickBtn{
    ...
    moneyLable.text = [NSString stringWithFormat:@"%@分",clickBtn.valueStr];
    ...
    moneyLable.frame =CGRectMake(0, 0, showView.frame.size.width, 30);
    moneyLable.textAlignment = NSTextAlignmentCenter;
//    [showView addSubview:yearLable];66 179 227
...
-(int )getUnitWithMaxValue:(int )maxValue{
    ...
    self.level = tempValue;
    self.unitYStr = @"我的成绩与省控线";
    if(self.level == 0){
        self.level = 1;

...
#import "JYTutorMyExpModel.h"
@interface JYTutorMyExpViewController()<UITableViewDataSource,UITableViewDelegate>
/// 存储数据
...
@implementation JYTutorMyExpViewController
...
- (void)viewDidLoad
{
    ...
    // 先取得前20条数据
    [[JYNetWorkTool sharedTools]request:JYRequestMethodGET urlString:@"http://api.dev.gaokaoq.com/AdvisorCenter/exp?p=1&pageSize=20" parameters:nil callback:^(id responseObject, NSError *error) {
        
        
        NSMutableArray *Array = [JYTutorMyExpModel mj_objectArrayWithKeyValuesArray:responseObject[@"data"][@"list"]];
        
        for (JYTutorMyExpModel *model in Array) {
...
- (void)updateData
{
    ...
    [[JYNetWorkTool sharedTools]request:JYRequestMethodGET urlString:@"http://api.dev.gaokaoq.com/user_center/exp?p=1&pageSize=20" parameters:nil callback:^(id responseObject, NSError *error) {
        ...
        NSArray *Array = [JYTutorMyExpModel mj_objectArrayWithKeyValuesArray:responseObject[@"data"][@"list"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    JYTutorMyExpModel *model = self.DataArray[indexPath.row];
